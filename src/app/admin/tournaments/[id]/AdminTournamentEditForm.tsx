...
type AdminTournamentEditFormProps = {
...
export default function AdminTournamentEditForm({
  tournament,
  isFreeSeason,
  freePrizePool,
}: AdminTournamentEditFormProps) {
  ...
  const [status, setStatus] = useState<
    "idle" | "saving" | "deleting" | "bracket" | "games" | "sync" | "error"
  >("idle");
  ...
  async function generateBracket() {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => null);
        setError(data?.error ?? "Could not generate bracket");
        setStatus("error");
        return;
      }
  ...
  async function createGames() {
    ...
    try {
      const response = await fetch(
        `/api/tournaments/${tournament.id}/lichess/create-games`,
        { method: "POST" },
      );
      if (!response.ok) {
        const data = await response.json().catch(() => null);
        setError(data?.error ?? "Could not create games");
        setStatus("error");
        return;
      }
      setStatus("idle");
  ...
  async function syncResults() {
    ...
    try {
      const response = await fetch(
        `/api/tournaments/${tournament.id}/lichess/sync-results`,
        { method: "POST" },
      );
      if (!response.ok) {
        const data = await response.json().catch(() => null);
        setError(data?.error ?? "Could not sync results");
        setStatus("error");
        return;
      }
      setStatus("idle");
